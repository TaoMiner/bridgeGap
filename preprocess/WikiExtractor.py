...
# not ok, because there are punctuations in the entity title
# punc_except = re.compile('[%s]' % re.escape('[]|():'))
# str_punc = re.sub(punc_except,'',string.punctuation)
...
class Extractor(object):
    ...
    def __init__(self, id, title, lines):
        ...
        # store the neighbor entity by outlinks
        self.outlinks = set()

    def extract(self, out, out_kg, out_id):
        """
        ...
        footer = "\n</doc>\n"
        #out.write(header)
        # output the wiki id file, replace the tab in title with space
        title_id = '%s\t%s\n' % (self.title.replace('\t', ' '), self.id)
        out_id.write(title_id.encode('utf-8'))
        for line in compact(text):
            cur = 0
            res = ''
            for s, e in findBalanced(line):
                tmp_line = punc.sub(' ', line[cur:s])
                tmp_line = tmp_line.replace('\t', ' ')
                tmp_line = re.sub(r'[\s]+', ' ', tmp_line)
                tmp_line = re.sub(r'(?<=\s)(\d+)(?=($|\s))', 'dddddd', tmp_line)
                tmp_line = re.sub(r'(?<=^)(\d+)(?=($|\s))', 'dddddd', tmp_line).lower()
                res += tmp_line + line[s:e]
                tmp_anchor = line[s:e]
                ...
                if len(tmp_title)>0:
                    self.outlinks.add(tmp_title)
                cur = e
            tmp_line = punc.sub(' ', line[cur:])
            tmp_line = tmp_line.replace('\t', ' ')
            tmp_line = re.sub(r'[\s]+', ' ', tmp_line)
            tmp_line = re.sub(r'(?<=\s)(\d+)(?=($|\s))', 'dddddd', tmp_line)
            tmp_line = re.sub(r'(?<=^)(\d+)(?=($|\s))', 'dddddd', tmp_line).lower()
            ...
            out.write('\n')
        # output the wiki page outlinks
        outlinks_text = '%s\t%s\n' % (self.title, '\t'.join(self.outlinks))
        out_kg.write(outlinks_text.encode('utf-8'))
        #out.write(footer)
...
def replaceInternalLinks(text):
    ...
    for s, e in findBalanced(text):
        ...
        while s-1>=cur and (u'\u0041' <= text[s-1]<=u'\u005a') or (u'\u0061' <= text[s-1]<=u'\u007a'):
            s -= 1
        res += text[cur:s]
        if s > 0 and text[s-1]!=' ':
            res += ' '
        # clean the label, punctuation, num, and white spaces
        label = label.replace('\t', ' ')
        label = punc.sub(' ', label)
        label = re.sub(r'[\s]+', ' ', label)
        label = re.sub(r'(?<=\s)(\d+)(?=($|\s))', 'dddddd', label)
        label = re.sub(r'(?<=^)(\d+)(?=($|\s))', 'dddddd', label).lower()
        res += makeInternalLink(title.replace('\t', ' '), label) #+ trail
        cur = end
    return res + text[cur:]
...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    output_queue_kg = Queue(maxsize=maxsize)
    output_queue_id = Queue(maxsize=maxsize)
    ...
    reduce_kg = Process(target=reduce_process,
                     args=(output_queue_kg, spool_length,
                           out_file+'/kg', file_size, file_compress))
    reduce_id = Process(target=reduce_process,
                     args=(output_queue_id, spool_length,
                           out_file+'/id', file_size, file_compress))
    reduce.start()
    reduce_kg.start()
    reduce_id.start()
    ...
    for i in xrange(worker_count):
        extractor = Process(target=extract_process,
                            args=(i, jobs_queue, output_queue, output_queue_kg, output_queue_id))
    ...
    output_queue_kg.put(None)
    output_queue_id.put(None)
...
def extract_process(i, jobs_queue, output_queue, output_queue_kg, output_queue_id):
    ...
    while True:
        ...
        if job:
            ...
            try:
                ...
                page = None              # free memory
                e.extract(out, out_kg, out_id)
                text = out.getvalue()
                text_kg = out_kg.getvalue()
                text_id = out_id.getvalue()
            except:
                text = ''
                text_kg = ''
                text_id = ''
            ...
            output_queue_kg.put((page_num, text_kg))
            output_queue_id.put((page_num, text_id))
            out.truncate(0)
            out_kg.truncate(0)
            out_id.truncate(0)
        else:
...
def main():
    ...
    global templateCache, escape_doc, redirects
