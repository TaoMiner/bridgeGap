...
int local_iter=0, binary = 1, debug_mode = 2, window = 5, min_count = 0, num_threads = 12, min_reduce = 1, max_sense_num = 10, text_sense = 1;
...
int main(int argc, char **argv) {
    ...
    if (read_text_path[0] != 0) LoadWordVector();
        ...
        if (save_text_path[0] != 0) SaveWordVector(0);
        //reset learning rate
        text_model.alpha = alpha;
    }
    ...
    if (read_kg_path[0] != 0) LoadEntityVector();
        ...
        if (save_kg_path[0] != 0) SaveEntityVector(0);
        kg_model.alpha = alpha;
    }
