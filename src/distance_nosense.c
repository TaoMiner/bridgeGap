...
void LoadTitleVector(){
    ...
    a = posix_memalign((void **)&(titles.et_vec), 128, (long long)vocab_size * layer_size * sizeof(real));
    if (titles.et_vec == NULL) {printf("Memory allocation failed\n"); exit(1);}
    
    //title entity cluster size
    titles.et_cluster_size = (int *)calloc(vocab_size, sizeof(int));
    ...
    for (b = 0; b < vocab_size; b++) {
        ...
        for (c = 0; c < layer_size; c++) fread(&titles.et_vec[c + title_index * layer_size], sizeof(float), 1, f);
        //norm
        len = 0;
        for (c = 0; c < layer_size; c++) len += titles.et_vec[c + title_index * layer_size] * titles.et_vec[c + title_index * layer_size];
        len = sqrt(len);
        for (c = 0; c < layer_size; c++) titles.et_vec[c + title_index * layer_size] /= len;
        

        titles.et_cluster_size[title_index] = 1;
...
void FindNearest(int top_n, float *vec){
    ...
    for (c = 0; c < entities.vocab_size; c++) {
        ...
        //if (index == c) continue;
        dist = 0;
        for (a = 0; a < layer_size; a++) dist += vec[a] * entities.vec[a + c * layer_size];
        for (a = 0; a < top_n; a++) {
            if (dist > bestd[a]) {
                for (d = top_n - 1; d > a; d--) {
                    bestd[d] = bestd[d - 1];
...
int main(int argc, char **argv) {
    ...
    while(1){
        ...
        if(title_index!=-1 && titles.et_cluster_size[title_index]>0){
            for (a = 0; a < layer_size; a++) word_vec[a] = 0;
            for (a = 0; a < layer_size; a++) word_vec[a] += titles.et_vec[a + title_index * layer_size];
            printf("\nTitle: %s  Position in vocabulary: %d\n", item, title_index);
