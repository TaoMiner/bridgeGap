...
int local_iter=0, binary = 1, debug_mode = 2, window = 5, min_count = 0, num_threads = 12, min_reduce = 1, max_sense_num = 10;
long long layer1_size = 100;
long long iter = 5, tmp_anchor_num = 0;
real alpha = 0.025, sample = 1e-3, cluster_threshold = -0.5;
real *expTable;
clock_t start;
char read_text_path[MAX_STRING], read_kg_path[MAX_STRING], output_path[MAX_STRING], save_text_path[MAX_STRING], save_kg_path[MAX_STRING];
int negative = 5;
...
void *TrainJointModelThread(void *id) {
    ...
    while (1) {
        ...
        //use cbow to align title embedding and entity according to the anchors
        for(anchor_position=0;anchor_position<anchor_count;anchor_position++){
            ...
            for (a = b; a < window * 2 + 1 - b; a++){
                if(a == window)
                    ...
                    last_word = sen[c];
                    if (last_word == -1) continue;
                    for (c = 0; c < layer1_size; c++) neu1[c] += text_model.syn0[c + last_word * layer1_size];
                    cw++;
                }
            }
            //title entity embedding
            for (c = 0; c < layer1_size; c++) neu1[c] += joint_model.et_syn0[c + anchors[anchor_position].entity_index * layer1_size];
            cw++;
            ...
            if (negative > 0) for (d = 0; d < negative + 1; d++) {
                ...
                for (c = 0; c < layer1_size; c++) f += neu1[c] * kg_model.syn1neg[c + l2];
                ...
                for (c = 0; c < layer1_size; c++) neu1e[c] += g * kg_model.syn1neg[c + l2];
                for (c = 0; c < layer1_size; c++) kg_model.syn1neg[c + l2] += g * neu1[c];
            }
            //reset the context vec
            ...
            for (a = b; a < window * 2 + 1 - b; a++){
                if(a == window)
                    ...
                    for (c = 0; c < layer1_size; c++) {
                        text_model.syn0[c + last_word * layer1_size] += neu1e[c];
                        //current context vec
            ...
            for (c = 0; c < layer1_size; c++) joint_model.et_syn0[c + anchors[anchor_position].entity_index * layer1_size] += neu1e[c];
            //update cluster
            if (cw>0){
                for(c = 0; c < layer1_size; c++)
                    joint_model.et_mu[c + anchors[anchor_position].entity_index * layer1_size] += tmp_context_vec[c]/cw;
...
void InitJointModel(){
    ...
    if (kg_model.vocab_size>0) readTitle(); else {printf("please init kg model first!");return;}
    InitTitleNet();
    if (tmp_anchor_num>0) printf("Total anchors in train file: %lld\n", tmp_anchor_num);
}
...
int main(int argc, char **argv) {
    ...
    InitKgModel();
    InitJointModel();
    //pre-train or load embeddings
    if (read_text_path[0] != 0) LoadWordVector();
    else {
        local_iter = 0;
        while(local_iter<iter){
            local_iter++;
            printf("Start training word embeddings the %d time... ", local_iter);
            TrainTextModel();
        }
        if (save_text_path[0] != 0) SaveWordVector();
    }
    
    if (read_kg_path[0] != 0) LoadEntityVector();
    else {
        local_iter = 0;
        while(local_iter<iter){
            local_iter++;
            printf("Start training entity embeddings the %d time... ", local_iter);
            TrainKgModel();
        }
        if (save_kg_path[0] != 0) SaveEntityVector();
    }
    ...
    while(local_iter<iter){
        local_iter++;
        printf("Start training joint model the %d time... ", local_iter);
        TrainJointModel();
    }
    printf("saving results...\n");
    SaveTitleVector(local_iter);
    sprintf(text_model.output_file, "%svectors_word.dat", output_path);
    SaveWordVector();
    return 0;
}
