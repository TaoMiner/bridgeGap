        ...
        with codecs.open(file, 'r', 'utf-8') as fin:
            for line in fin:
                ...
                if len(items) == 2 and items[1] in self.wiki_id and len(items[0]) > 0:
                    tmp_ments = set() if items[0] not in self.names else self.names[items[0]]
                    tmp_ments.add(self.wiki_id[items[1]])
                    self.names[items[0]] = tmp_ments
            print 'successfully load %d lower maps!' % len(self.names)
        ...
        with codecs.open(file, 'r', 'utf-8') as fin:
            for line in fin:
                ...
                for item in items[2:]:
                    ...
                    tmp_ments = set() if tmp_items[0] not in self.names else self.names[tmp_items[0]]
                    tmp_ments.add(items[0])
                    self.names[tmp_items[0]] = tmp_ments
        print 'successfully load %d anchor mentions!' % len(self.names)
